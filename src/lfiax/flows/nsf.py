...
def make_nsf(
    ...
    if event_shape == (1,):
        # Scalar event: the only data dim must be transformed (mask=False),
        # not held back as conditioning context (mask=True). With mask=True
        # the coupling layer is identity for the only dim, and the flow
        # degenerates to its base distribution — log_prob is constant in y
        # and θ, so EIG and posterior reweighting both collapse.
        mask = jnp.array([0]).astype(bool)
