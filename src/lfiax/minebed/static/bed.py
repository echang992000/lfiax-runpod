...
# GPy/GPyOpt releases used by MINEBED still reference np.bool, which was
# removed in NumPy >= 1.24. Patch the alias before importing GPyOpt.
...
class GradientFreeBED(BED):
    ...
    def train(
        ...
        BO_max_num: int
            ...
            (default is 20)
        eps: float
            Minimum distance between consecutive BO suggestions before GPyOpt
            stops early. Set to -np.inf to disable this stopping condition.
            (default is 1e-5)
        verbosity: boolean
        ...
        self.bo_obj.run_optimization(
            max_iter=BO_max_num, verbosity=verbosity, eps=eps)
