...
from lfiax.utils.oed_losses import lf_pce_eig_scan, snpe_c
...
def make_bmp_prior():
    ...
    uniform = distrax.Uniform(low=jnp.array([1e-6]), high=jnp.array([1.0]))
    
    forward_fn = lambda x: jnp.exp(x * (high - low) + low)
    inverse_fn = lambda y: (jnp.log(y) - low) / (high - low)
    
    log_uniform = distrax.Transformed(
        uniform, bijector=distrax.Lambda(forward=forward_fn, inverse=inverse_fn)
    )
    bmp_prior = distrax.Independent(log_uniform, reinterpreted_batch_ndims=1)
...
class Workspace:
    def __init__(self, cfg):
        ...
        file_name = f"eig_lambda_{eig_lambda_str}_random"
        self.subdir = os.path.join(
            os.getcwd(), "icml_2024", 'BMP_random', cfg.designs.norm_type, file_name, str(cfg.designs.num_xi), str(cfg.seed), current_time_str
            )
    ...
    def run(self) -> Callable:
        ...
        def update_pce(
            ...
            (loss, (conditional_lp, EIG)), grads = jax.value_and_grad(
                ...
                post_params, xi_params, self.d, prng_key, scaled_x, theta_0,
            ...
            return new_params, xi_params, new_opt_state, opt_state_xi, loss, conditional_lp, EIG
        ...
        optimizer2 = optax.adam(learning_rate=1e-3)
        ...
        params = {key: value for key, value in params.items() if key != 'xi'}

        priors = make_bmp_prior()
        total_EIG = 0

        for design_round in range(5):
            max_EIG = 0
            for step in range(self.training_steps):
                ...
                params, xi_params_max_norm, opt_state, opt_state_xi, loss, conditional_lp, EIG = update_pce(
                    params, xi_params_max_norm, next(prng_seq), opt_state, opt_state_xi, priors, scaled_x, theta_0=theta_0.squeeze(), N=self.N, M=self.M, lam=self.eig_lambda
                )
                ...
                Loss: {float(loss):.5f}; EIG: {float(EIG):.5f}; Inference Time: {inference_time:.5f} \
                ...
                writer.writerow({
                    ...
                    'simulate_time':float(simulate_time)
                })
                logf.flush()
                
                if self.cfg.wandb.use_wandb:
                    wandb.log({"loss": loss, "xi": xi_params['xi'], "EIG": EIG, "simulation_time": simulate_time, "inference_time": inference_time})
            
            self.xi = distrax.Uniform(low=0., high=1e3).sample(seed=next(prng_seq), sample_shape=(self.cfg.designs.num_xi,1))
    ...
    def _init_logging(self):
        ...
        logf = open(path, 'a') 
        fieldnames = ['seed', 'STEP', 'Xi', 'Loss', 'EIG', 'inference_time', 'simulate_time']
...
from BMP_random import Workspace as W
