...
class Workspace:
    def __init__(self, cfg):
        ...
        self.subdir = os.path.join(os.getcwd(), "iclr", 'BMP_unif', cfg.designs.norm_type, file_name, str(cfg.designs.num_xi), str(cfg.seed), current_time_str)
        os.makedirs(self.subdir, exist_ok=True)

        self.seed = self.cfg.seed
        rng = jrandom.PRNGKey(self.seed)

        # Set the range of the log-uniform distribution
        min_val = 1e-6
        max_val = 1e3

        # Specify the number of samples
        num_samples = cfg.designs.num_xi

        # Generate equidistant log-uniform samples
        self.xi = jnp.geomspace(min_val, max_val, num_samples)[None,:]
        self.d_sim = self.xi
    ...
    def run(self) -> Callable:
        ...
        for step in range(self.training_steps):
            ...
            post_params, xi_params_max_norm, opt_state, opt_state_xi, loss, conditional_lp, EIG = update_snpe_pce(
        ...
        objects = {
            'post_params': jax.device_get(post_params),
    ...
    def _init_logging(self):
        ...
        logf = open(path, 'a') 
        fieldnames = ['seed', 'STEP', 'Loss', 'EIG', 'inference_time', 'simulate_time']
...
from BMP_uniform import Workspace as W
