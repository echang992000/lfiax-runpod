...
from lfiax.utils.utils import standard_scale, compute_gauss_cdf_value, reverse_gauss_scale, compute_t_value, reverse_t_value
...
class Workspace:
    def __init__(self, cfg):
        ...
        # TODO: Check that this is working properly... and delete.
        ...
        self.subdir = os.path.join(os.getcwd(), "iclr", 'snpe_pce_lin_reg', file_name, str(cfg.designs.num_xi), str(cfg.seed), current_time_str)
    ...
    def run(self) -> Callable:
        ...
        if norm_type == "inf":
            ...
            xi_params_scaled = {}
            xi_params_scaled['xi'] = compute_gauss_cdf_value(xi_params['xi'], scale_factor)
        elif norm_type == "student":
            xi_params_scaled = {}
            xi_params_scaled['xi'] = compute_t_value(xi_params['xi'], scale_factor, df=df)
        else:
        ...
        for step in range(self.training_steps):
            ...
            if norm_type == "inf":
                ...
                xi_params['xi'] = reverse_gauss_scale(xi_params_scaled['xi'], scale_factor)
                # clip the values
                ...
                # Unnormalize
                xi_params['xi'] = reverse_t_value(xi_params_scaled['xi'], scale_factor, df)
                # clip the values
                xi_params['xi'] = jnp.clip(
                    xi_params['xi'], 
                    a_min=design_min, 
                ...
                # Re-normalize
                xi_params_scaled['xi'] = compute_t_value(xi_params['xi'], scale_factor, df)
            else:
                raise ValueError(f"Norm type {norm_type} not recognized.")
            ...
            if self.cfg.wandb.use_wandb:
                wandb.log({
                    "norm type": self.cfg.designs.norm_type,
                    "loss": loss, 
                    "xi": xi_params['xi'], 
